...
class PD(NOX.Epetra.Interface.Required,
    ...
    def __init__(self, num_nodes, length, width=10.0, bc_regions=None,
        ...
        if horizon != None:
            ...
            self.horizon = 2.1 * self.grid_spacing
    ...
    def __init_grid_data(self):
        ...
        x_min_right = np.where(self.my_x >= l-(1.0*gs+hgs))
        ...
        ymax_bottom = np.where(self.my_y <= (1.0*gs+hgs))[0]
        ...
        ymin_top = np.where(self.my_y >= l-(1.0*gs+hgs))[0]
        ...
        #print (global_overlap_indices)
        # ttt.sleep(2)
        # if self.rank==0:
        #    print "shit"
        # else:
        #    print (global_overlap_indices.shape[0])
        #    ttt.sleep(1)
        # ttt.sleep(3)
    ...
    def compute_flow(self, ux, flow, trans, uy, flag):
        """
        ...
        saturation_state = ma.masked_array(
            uy[neighbors] - uy[:num_owned, None], mask=neighbors.mask)
        pressure_state = ma.masked_array(
            ux[neighbors] - ux[:num_owned, None], mask=neighbors.mask)
        ...
        residual = -1.0 * term_contributions
        trans[:] = 0.0
        ...
        term_x[1:-1, 1:-1] = (my_ux[1:-1, 2:] - 2*my_ux[1:-1, 1:-1]+my_ux[1:-1, 0:-2])/(self.delta_x**2)+9.81
        term_y[1:-1, 1:-1] = (my_uy[1:-1, 2:] - 2*my_uy[1:-1, 1:-1]+my_uy[1:-1, 0:-2])/(self.delta_x**2)+9.81
        ...
        residual_ux_overlap = -term_x_unsorted[:num_owned]
        residual_uy_overlap = -term_y_unsorted[:num_owned]

        flow_residual[:] = 0.0
        trans_residual[:] = 0.0
        flow_residual[:num_owned] += residual_ux_overlap
        trans_residual[:num_owned] += residual_uy_overlap
        """

        """

        # Calculate Pressure based on current velocity vslues using divergence free condition
        current_feshar = my_ux
        current_feshar[1:-1, 1:-1] = self.feshar_const * ((my_ux[1:-1, :-2] - my_ux[1:-1, 2:]) / 2.0 / self.delta_x + (
            my_uy[:-2, 1:-1] - my_uy[2:, 1:-1]) / 2.0 / self.delta_y)
        # Zeroth term of residual is the time derivative
        ...
        # First term, u · ∇u term, with central difference approximation of gradient
        ...
        term_x_unsorted = np.append(x_flat[x_stride:], ghost_val_x)
        term_y_unsorted = np.append(y_flat[x_stride:], ghost_val_y)
        self.residual_overlap[:-1:2] += term_x_unsorted
        self.residual_overlap[1::2] += term_y_unsorted
        #self.residual[:-1:2] = self.residual_overlap[:num_owned]

        # second term, viscous term ,calculation
        term_x[1:-1, 1:-1] = self.visc * ((my_ux[1:-1, 2:] - 2*my_ux[1:-1, 1:-1]+my_ux[1:-1, 0:-2])/(
            self.delta_x**2) + ((my_ux[2:,  1:-1]-2*my_ux[1:-1, 1:-1]+my_ux[:-2, 1:-1])/self.delta_y**2.0))
        term_y[1:-1, 1:-1] = self.visc * ((my_uy[1:-1, 2:] - 2*my_uy[1:-1, 1:-1]+my_uy[1:-1, 0:-2])/(
            self.delta_x**2) + ((my_uy[2:,   1:-1]-2*my_uy[1:-1, 1:-1]+my_uy[:-2, 1:-1])/self.delta_y**2.0))
        # Add these terms into the residual
        x_flat = term_x.flatten()
        y_flat = term_y.flatten()
        ####resorting to put ghost nodes last
    ...
    def computeF(self, x, F, flag):
        ...
        try:
            ...
            self.compute_flow(my_ux_overlap, self.my_flow_overlap,
                              self.my_trans_overlap, my_uy_overlap, flag)
            ...
            self.F_fill_overlap[ux_local_overlap_indices] = self.my_flow_overlap
            self.F_fill_overlap[uy_local_overlap_indices] = self.my_trans_overlap
